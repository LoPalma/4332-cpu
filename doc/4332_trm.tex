...
Despite its simplicity the 4332 does not compromise on system features. It has a proper privilege architecture, a vectored interrupt controller with 8 vectors, and a 3-stage pipeline --- all in a footprint of under 570 LUTs on an Artix-7.
...
\begin{itemize}
    ...
    \item \textbf{3-stage pipeline.}
          FETCH, DECODE, and EXECUTE stages. RAW hazards are resolved by a one-cycle stall with bubble injection. Branches flush the pipeline on taken, producing a single-cycle penalty bubble. There is deliberately no branch predictor --- the pipeline is short enough that prediction overhead outweighs any potential gain.
...
Options:
  ...
  -f, --format FORMAT  Output format: bin | hex | textio (default: bin)
\end{lstlisting}
